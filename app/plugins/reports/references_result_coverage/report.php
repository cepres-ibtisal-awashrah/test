...
/**
 * Reference Result Coverage report for TestRail
 *
 * Copyright Gurock Software GmbH. All rights reserved.
 *
 * This is the TestRail report for displaying the test result coverage
 * for references in a coverage matrix.
 *
 * http://www.gurock.com/testrail/
 */

class References_result_coverage_report_plugin extends Report_plugin
{
	private $_controls;
	...
	public function prepare_form($context, $validation)
	{
		...
		$validation->add_rules(
			array(
				...
				'custom_cases_include_coverage' => array(
					'type' => 'bool',
					'default' => false
				)
			)
	...
	{
		// At least one case option must be selected (coverage or
		// comparison).
		if (!$input['custom_cases_include_coverage'] &&
			!$input['custom_cases_include_comparison'])
		{
			$validation->add_error(
				lang('reports_rrc_form_cases_required')
			);
	...
	public function run($context, $options)
	{
		...
		// We then read the test suites for the case IDs of these
		// references. We do this to limit the test result/run scope
		// to those runs that really affect this report.
		if ($case_ids)
		{
		...
		// And finally read the results (comparison)/results (latest/
		// coverage), depending on the report options.
		$show_coverage = $options['cases_include_coverage'];
		if ($show_coverage && $run_ids)
		{
			$results_latest = 
		...
		if ($run_ids)
		{
			$results = $this->_helper->get_results_for_cases_many(
				$run_ids,
				$case_ids,
				$runs,
				$runs_with_results,

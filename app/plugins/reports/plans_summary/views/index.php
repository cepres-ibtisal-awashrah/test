...
<h1 class="main">
	<span><?php echo  langf('reports_pls_title', h($plan->name))?></span>
</h1>

<?php tests::set_status_percents($plan) ?>
...
<?php if ($plan->description): ?>
	<div class="markdown" style="<?php echo  !$status_include ? 'margin: 1.5em 0' : '' ?>">
		<?php if ($show_links): ?>
		<?php echo  markdown::to_html($plan->description) ?>
		<?php else: ?>
		<?php echo  markdown::to_html_nolinks($plan->description) ?>
		<?php endif ?>
	</div>
<?php endif ?>
...
<h1 class="not-allow-break"><img class="right noPrint w16" src="%RESOURCE%:images/report-assets/help.svg" alt="" title="<?php echo  lang('reports_pls_runs_info') ?>" /><?php echo  lang('reports_pls_runs') ?></h1>
...
<?php if ($runs): ?>
	...
	?>
<?php else: ?>
	<p><?php echo  lang('reports_pls_runs_empty') ?></p>
<?php endif ?>

<?php if ($activities_include): ?>
	<div class="chart-wrapper-outter">
		<div class="chart-wrapper-inner">
			<h1><img class="right noPrint w16" src="%RESOURCE%:images/report-assets/help.svg" alt="" title="<?php echo  lang('reports_pls_activity_info') ?>" /><?php echo  lang('reports_pls_activity') ?></h1>
			<?php if ($activity): ?>
				<?php
				$temp = [];
			...
			<?php if ($activities): ?>
				...
				<p class="partial">
				<?php echo  langf('reports_pls_activity_more', $activities_limit) ?>
				</p>
				<?php endif ?>
			<?php else: ?>
				<p class="top"><?php echo  lang('reports_pls_activity_empty') ?></p>
			<?php endif ?>
		</div>
	</div>
<?php endif ?>
...
<?php if ($progress_include && $progress): ?>
	<div class="chart-wrapper-outter">
		<div class="chart-wrapper-inner">
			<h1><img class="right noPrint w16" src="%RESOURCE%:images/report-assets/help.svg" alt="" title="<?php echo  lang('reports_pls_progress_info') ?>" /><?php echo  lang('reports_pls_progress') ?></h1>
			<?php if ($burndown): ?>
				<?php
				$temp = [];
	...
	<h2 class="top"><?php echo  lang('reports_pls_progress_forecasts') ?></h2>
	<?php
	$temp = [];
	$temp['plan'] = $plan;
	$temp['progress'] = $progress;
	...
	?>
	<?php if ($runs): ?>
		<h2><?php echo  lang('reports_pls_progress_runs') ?></h2>
		<?php
		$temp = [];
		$temp['runs'] = $runs;
		$temp['show_links'] = $show_links;
...
<?php if ($tests_include && $runs): ?>
	...
	<h1><img class="right noPrint w16" src="%RESOURCE%:images/report-assets/help.svg" alt="" title="<?php echo  lang('reports_pls_tests_info') ?>" /><?php echo  lang('reports_pls_tests') ?></h1>
	<?php foreach ($runs as $run): ?>
	...
	<p class="partial">
		<?php echo  langf('reports_pls_tests_more', $test_limit) ?>
	</p>
	<?php endif ?>
	<?php if (!$has_tests): ?>
		<p class="top"><?php echo  lang('reports_pls_tests_empty') ?></p>
	<?php endif ?>
<?php endif ?>

...
if ($show_comparison && $show_summary) {
    $title = lang('reports_rds_charts_bar_title_full');
} else if ($show_comparison) {
    $title = lang('reports_rds_charts_bar_title_comparison');
} else {
    $title = lang('reports_rds_charts_bar_title_summary');
}
...
if ($show_comparison) {
    ...
    if ($show_summary) {
        ...
        $categories[] = ['label' => lang('reports_rds_defects_summary')];
    }
} else {
    $data[] = ['value' => $summary->defect_count];
    $references_data[] = ['value' => $references->reference_count];
    $categories[] = ['label' => ''];
}
...
<div class="chartContainer">
	<div id="chart0" style="height: 250px;"><?php echo  $encodedImage ?? '' ?></div>
</div>

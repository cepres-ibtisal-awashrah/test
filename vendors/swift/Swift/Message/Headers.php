...
class Swift_Message_Headers
{
  ...
  public function getEncoded($name)
  {
    ...
    foreach ($encoded_value as $key => $row)
    {
      ...
      // GUROCK: added
      // Force encoding for From/Reply-To headers with colon. Saw email formatting/
      // encoding issues with those headers otherwise (invalid senders etc.). 

      $is_header_safe = Swift_Message_Encoder::instance()->isHeaderSafe($row);
      if ($is_header_safe)
      {
        if ($lname == 'from' || $lname == 'reply-to')
        {
          $is_header_safe = strpos($row, ":") === false;
        }
      }

      //If the header is 7-bit printable it's at no risk of injection
      if ($is_header_safe && !$this->forceEncoding)
      {

...
{
    protected const SET_IDENTITY = 'SET IDENTITY_INSERT phinxlog ';
    protected const DECL_TYPE = 'sqlsrv:decl_type';
    /**
     * @var \PDO|null
     */
    ...
    public function setConnection(PDO $connection)
    {
        ...
        if (!$this->hasSchemaTable()) {
            ...
            $isMsSqlInsertToPhinxlog = $this->getAdapterType() === AbstractAdapter::WINDOWS_DATABASE
                && $this->getSchemaTableName() === AbstractAdapter::PHINX_LOG_TABLE_NAME
                && $this->isPhinxlogIdentityEnabled();
            if ($isMsSqlInsertToPhinxlog) {
                $this->execute(static::SET_IDENTITY . 'ON');
            }
            ...
            if ($isMsSqlInsertToPhinxlog) {
                $this->execute(static::SET_IDENTITY . 'OFF');
            }
        }
    ...
    /**
     * Checks if SET IDENTITY_INSERT ON/OFF is valid for phinxlog table
     *
     * @return bool
     */
    protected function isPhinxlogIdentityEnabled(): bool
    {
        $stmt = $this->query('SELECT version FROM ' . AbstractAdapter::PHINX_LOG_TABLE_NAME);
        $metadata = $stmt->getColumnMeta(0);

        return isset($metadata[static::DECL_TYPE]) && strpos($metadata[static::DECL_TYPE], 'identity') !== false;
    }
